...
import { Box,  Button,  Center,  CloseButton, DownloadTrigger, Drawer, Field, Flex, Heading, HStack, IconButton, Image, Input, Link, Portal, RadioGroup, Skeleton, Stack, Table, Tag, VStack} from "@chakra-ui/react"
...
const RelatoriosTransacional = ({data, error, isLoading, usuario}:Relatorios) => {
        ...
        <Drawer.Positioner>
          <Drawer.Content>
            <Drawer.Header bg={COLORS.amarelo} color={COLORS.preto} borderBottom={`5px solid ${COLORS.vermelho}`} >
              <Drawer.Title fontWeight={300}>Gerar relatórios de pedidos de contação</Drawer.Title>
            </Drawer.Header>
            <Drawer.Body p={10}>
                                ...
                                <Table.Body>
                                    ...
                                    <Table.Row fontSize={12} key={item.id_relatorio}>
                                        ...
                                        <Table.Cell textAlign="center">
                                            <Link cursor="pointer" href={`${BaseInfo.baseURL}reports/${item.file_gerado}`} download={true} p={2} bg={COLORS.vermelho} rounded={5}>
