  ...
  return (
    <Drawer.Root size="sm" open={open} onOpenChange={(e) => setOpen(e.open)}>
      <Drawer.Trigger asChild>
        <IconButton onClick={pay} ml={2} p={3} size={"xs"} bg={"green.600"} color={COLORS.branco} rounded={5}>
            <GiTakeMyMoney />
        </IconButton>
      </Drawer.Trigger>
      <Portal>
        ...
        <Drawer.Positioner>
          <Drawer.Content>
            <Drawer.Header bg={COLORS.amarelo} color={COLORS.preto} borderBottom={`5px solid ${COLORS.vermelho}`} >
              <Drawer.Title fontWeight={300}>Liquidar factura</Drawer.Title>
            </Drawer.Header>
            <Drawer.Body p={10}>
              <Heading textAlign="center" fontSize={40} mt={3}># {data.facturaId}</Heading>
              <Box mt={5} fontSize={14} fontWeight={300}>
                <Flex mb={5} justifyContent="center">
                  <Text fontWeight={500} textAlign="center">Deseja realmente liquidar a factura?</Text>
                </Flex>
                <Flex mb={5} justifyContent="center">
                  <Button bg={COLORS.amarelo} loading={load} onClick={()=>exclusao(data.id_referencia)} color={COLORS.preto} width={[150]} rounded={10}>Pagar</Button>
