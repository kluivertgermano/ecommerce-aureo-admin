  ...
  const [load, setLoad] = useState(false)
    ...
    const exclusao = async (id:string) => {
        ...
        try {
          ...
          const response = await RequestAPI.delete(`produtos/${id}`)
        ...
        <Drawer.Positioner>
          <Drawer.Content>
            <Drawer.Header bg={COLORS.amarelo} color={COLORS.preto} borderBottom={`5px solid ${COLORS.vermelho}`} >
              <Drawer.Title fontWeight={300}>Exclusão de produto</Drawer.Title>
            </Drawer.Header>
            <Drawer.Body p={10}>
              <Heading textAlign="center" fontSize={40} mt={3}># {data.produtoId}</Heading>
              <Box mt={5} fontSize={14} fontWeight={300}>
                <Flex mb={5} justifyContent="center">
                  <Text fontWeight={500} textAlign="center">Deseja realmente excluir o produto?</Text>
                </Flex>
                <Flex mb={5} justifyContent="center">
                  <Button bg={COLORS.amarelo} loading={load} onClick={()=>exclusao(data.produtoId)} color={COLORS.preto} width={[150]} rounded={10}>Excluir</Button>
