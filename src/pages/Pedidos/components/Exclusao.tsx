  ...
  const [load, setLoad] = useState(false)
    ...
    const exclusao = async (id:string) => {
        ...
        try {
          ...
          const response = await RequestAPI.delete(`pedidos-cotacao/${id}`)
        ...
        <Drawer.Positioner>
          <Drawer.Content>
            <Drawer.Header bg={COLORS.amarelo} color={COLORS.preto} borderBottom={`5px solid ${COLORS.vermelho}`} >
              <Drawer.Title fontWeight={300}>Exclusão de pedido</Drawer.Title>
            </Drawer.Header>
            <Drawer.Body p={10}>
              <Heading textAlign="center" fontSize={40} mt={3}># {data.numero_cotacao}</Heading>
              <Box mt={5} fontSize={14} fontWeight={300}>
                <Flex mb={5} justifyContent="center">
                  <Text color={"gray.500"} fontWeight={500} textAlign="center">Deseja realmente excluir o pedido?</Text>
                </Flex>
                <Flex mb={5} justifyContent="center">
                  <Button bg={COLORS.amarelo} loading={load} onClick={()=>exclusao(data.pedidoCotacaoId)} color={COLORS.preto} width={[150]} rounded={10}>Excluir</Button>
